...
public class Stablize_SC {
    ...
    public static Picture bs(Picture picture, int[][] protect, int new_width, int new_height) {
        if (new_height > picture.height()) {
            picture = expandingHeight(picture, protect, new_height);
        }
        if (new_width > picture.width()) {
            picture = expandingWidth(picture, protect, new_width);
        }
        picture = shrinking(picture, protect, new_width, new_height);
        return picture;
    }
